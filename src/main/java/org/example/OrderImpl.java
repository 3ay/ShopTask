...
класс OrderImpl может быть расширен для добавления новых операций или функциональностей, связанных с заказами, без изменения существующего кода.
...
public class OrderImpl implements Order{
    ...
    @Override
    public void addProductToCustomerBucket(AbstractProduct product, int countOfProduct) {
        ...
        if (customerBucket.contains(product)) {
            for (Product productImplOnBucket : customerBucket) {
                if (productImplOnBucket.equals(product)) {
                    productImplOnBucket.setCountOfProduct(productImplOnBucket.getCountOfProduct() + 1);
                }
            }
        } else
    ...
    public double sumOfBucket(List<AbstractProduct> customerBucket) {
        ...
        for (AbstractProduct abstractProduct : customerBucket) {
            sum += abstractProduct.getPrice() * abstractProduct.getCountOfProduct();
        }
        return sum;
    }
